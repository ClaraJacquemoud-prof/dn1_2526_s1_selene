...
if(modeAléatoire == true){
    ...
    if(mouseIsPressed && mouseButton === LEFT && brushUtilisé && (mouseX > bordGauche && mouseX < bordDroit) && (mouseY > bordHaut && mouseY < bordBas)){
               let x = int(1+frameCount*0.01%(matiereBrush.length-2))
               print(x)
               let aleatoire = int(random(60))
               ...
               if(img){
                  image(matiereBrush[x], mouseX, mouseY, tailleBrush, tailleBrush)
              print('nlkn')
               }else{
                 image(imgBrush[x], mouseX, mouseY, tailleBrush, tailleBrush)
               }
